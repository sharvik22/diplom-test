...
* init-backend.sh
  чтобы иницилизтровать хранения стейт файла проекта, нужно в начале создать сам бакет.
  при выполнении кода у меня создается бакет, затем с помошью скрипта я иницилизирую храниение стейт файла проета
  для этой иницилизации мне нужно подключиться к бакету (иметь ACCESS_KEY и SECRET_KEY)
