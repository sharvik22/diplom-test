...
* export_network_config.sh - делает иницилизацию сети для второго проекта, т.к. буду использовать уже созданную сеть.
  
* init-backend.sh - чтобы иницилизтровать хранения стейт файла проекта, нужно в начале создать сам бакет.
  При выполнении кода у меня создается бакет, затем с помошью скрипта я иницилизирую храниение стейт файла проета для этой иницилизации мне нужно подключиться к бакету (иметь ACCESS_KEY и SECRET_KEY).
  Скрипт передает ключи из вывода, сохраняет их во временный файл, выводит мне для проверки и экспорта переменные и удаляет файл в котором есть сектретные ключи.
